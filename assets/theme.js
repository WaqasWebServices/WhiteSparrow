  ...
  function initCroFreeShipping() {
    ...
    function update(cart) {
      document.querySelectorAll('[data-free-shipping-progress]').forEach(function(el) {
        var threshold = parseInt(el.dataset.threshold, 10) || 7500;
        var remaining = threshold - cart.total_price;
        var percent = Math.min(Math.max(cart.total_price / threshold * 100, 0), 100);
        var message = el.querySelector('[data-free-shipping-message]');
        var bar = el.querySelector('[data-free-shipping-bar]');

        if (message) {
          message.textContent = remaining > 0 ? "You're " + money(remaining) + " away from free shipping." : "You've unlocked free shipping.";
        }
    ...
    if (!document.querySelector('[data-free-shipping-progress]')) return;

    theme.cart.getCart().then(update);

    document.addEventListener('cart:updated', function(evt) {
      if (evt.detail && evt.detail.cart) {
        update(evt.detail.cart);
      }
    });
  }

  function initCroStickyAtc() {
    var sticky = document.querySelector('[data-cro-sticky-atc]');
    var mainButton = document.querySelector('.product-single__form [data-add-to-cart]');

    if (!sticky || !mainButton) return;

    var stickyButton = sticky.querySelector('[data-cro-sticky-atc-button]');
    document.body.classList.add('has-cro-sticky-atc');
